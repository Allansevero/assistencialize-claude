...
func (s *server) connectOnStartup() {
	// rows, err := s.db.Queryx("SELECT id,token,jid,webhook,events FROM instances WHERE connected=1")
	rows, err := s.db.Queryx("SELECT id,token,jid,webhook,events,proxy_url FROM instances WHERE connected=1")
	if err != nil {
...
func (s *server) startClient(userID int, textjid string, token string, subscriptions []string) {
	...
	// NEW: set proxy if defined in DB (assumes instances table contains proxy_url column)
	var proxyURL string
	err = s.db.Get(&proxyURL, "SELECT proxy_url FROM instances WHERE id=$1", userID)
	if err == nil && proxyURL != "" {
		...
		if err != nil {
			...
			for evt := range qrChan {
				if evt.Event == "code" {
					...
					sqlStmt := `UPDATE instances SET qrcode=$1 WHERE id=$2`
					...
					sqlStmt := `UPDATE instances SET qrcode=$1, connected=1 WHERE id=$2`
					_, err := s.db.Exec(sqlStmt, "", userID)
					if err != nil {
						log.Error().Err(err).Msg(sqlStmt)
					}
	...
	for {
		...
		case <-killchannel[userID]:
			...
			client.Disconnect()
			delete(clientPointer, userID)
			sqlStmt := `UPDATE instances SET qrcode=$1, connected=0 WHERE id=$2`
			_, err := s.db.Exec(sqlStmt, "", userID)
			if err != nil {
				log.Error().Err(err).Msg(sqlStmt)
			}
...
func (mycli *MyClient) myEventHandler(rawEvt interface{}) {
	...
	case *events.Connected:
		...
		sqlStmt := `UPDATE instances SET connected=1 WHERE id=$1`
	...
	case *events.PairSuccess:
		...
		jid := evt.ID
		sqlStmt := `UPDATE instances SET jid=$1 WHERE id=$2`
	...
	case *events.LoggedOut:
		...
		sqlStmt := `UPDATE instances SET connected=0 WHERE id=$1`
		_, err := mycli.db.Exec(sqlStmt, mycli.userID)
		if err != nil {
			log.Error().Err(err).Msg(sqlStmt)
		}
